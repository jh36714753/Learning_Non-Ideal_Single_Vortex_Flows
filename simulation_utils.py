...
from learning_utils import calc_vort
from learning_utils import bilinear_interpolate
import math
import time
from functorch import vmap
import torch.nn.functional as F
from learning_utils import calc_laplacian
...
def apply_gaussian_filter(tensor, kernel_size=5, sigma=1.0):
    ...
    filtered = F.conv2d(tensor, kernel, padding=padding, groups=tensor.shape[1])
    
    return filtered.squeeze(0) if tensor.dim() == 3 else filtered
...
def simulate_step(img, img_x, vorts_pos, vorts_w, vorts_size, nu, omega, vel_func, dt, boundary):
    ...
    if boundary is None:
        ...
        vor = bilinear_interpolate(img_vor, vorts_pos)
        ...
        img_omega[:,:,:,2] = omega  # z
        img_b = -2* torch.cross(img_omega, img_vel_3d*vort_scale, dim=3)
        img_curl_force = calc_vort(img_b, boundary = boundary)
        curl_force = bilinear_interpolate(img_curl_force, vorts_pos)
        ...
        new_vorts_w = RK1(vorts_w, nu*calc_laplacian(vor) + curl_force, dt)
        vorts_size_square = vorts_size ** 2
        new_vorts_size_square = RK1(vorts_size_square, 4*nu, dt)
        new_vorts_size = torch.sqrt(new_vorts_size_square)
    else:
        ...
        img_b = -2* torch.cross(img_omega, img_vel_3d, dim=3)
        img_curl_force = calc_vort(img_b, boundary = boundary)
        curl_force = bilinear_interpolate(img_curl_force, vorts_pos)
    ...
    return new_img, new_vorts_pos, img_vel, vorts_vel, new_vorts_w, new_vorts_size
...
def simulate(img, img_x, vorts_pos, vorts_w, vorts_size, num_steps, nu, omega, vel_func, boundary = None, dt = dt_, ):
    imgs = []
    vorts_poss = []
    img_vels = []
    vorts_vels = []
    ...
    for i in range(num_steps):
        img, vorts_pos, img_vel, vorts_vel, vorts_w, vorts_size = simulate_step(img, img_x, vorts_pos, vorts_w, vorts_size, nu, omega, vel_func, dt, boundary = boundary)
        imgs.append(img.clone())
        vorts_poss.append(vorts_pos.clone())
        img_vels.append(img_vel)
        vorts_vels.append(vorts_vel)
        vorts_ww.append(vorts_w)
        vorts_size_.append(vorts_size)

    return imgs, vorts_poss, img_vels, vorts_vels, vorts_ww

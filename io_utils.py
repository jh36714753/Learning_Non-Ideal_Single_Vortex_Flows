...
# generate grid coordinates with (0.5, 0.5) as center
def gen_grid(width, height, device):
    img_n_grid_x = width
    img_n_grid_y = height
    img_dx = 1. / max(img_n_grid_x, img_n_grid_y)
    
    # Create meshgrid
    c_x, c_y = torch.meshgrid(
        torch.arange(img_n_grid_x), 
        torch.arange(img_n_grid_y), 
        indexing="ij"
    )
    
    # Center the coordinates around (0.5, 0.5)
    c_x = (c_x - (img_n_grid_x - 1) / 2.0) * img_dx + 0.5
    c_y = (c_y - (img_n_grid_y - 1) / 2.0) * img_dx + 0.5
    
    # Calculate grid center locations
    img_x = torch.cat((c_x[..., None], c_y[..., None]), axis=2).to(device)
...
# write vortex (particles) with their velocities
def write_vorts_1(vorts_pos, vorts_uv, outdir, i):
    vorts_pos = copy.deepcopy(vorts_pos)
    pos = vorts_pos
    fig = plt.figure(num=1, figsize=(7, 7), clear=True)
    ax = fig.add_subplot()
    ...
    s = ax.scatter(pos[..., 0], pos[..., 1], s = 100)
    ax.quiver(pos[..., 0], pos[..., 1], vorts_uv[..., 0], vorts_uv[..., 1], color = "red", scale = 10.)
    fig.savefig(os.path.join(outdir, '{:03d}_VP.jpg'.format(i)), dpi = 512//8)   
...
def write_vorticity(vort_img, outdir, i):
    ...
    #p = ax.imshow(array, alpha = 0.75, vmin = -0.25, vmax = 0.25, origin='lower')
...
def write_visualization(img, vorts_pos, vorts_w, outdir, i, boundary = None):
    ...
    #p = ax.imshow(array, alpha = 0.75, origin='lower')
...
def config_parser():
    ...
    parser.add_argument("--vort_w", type=float, default=0,
                        help='how vorticities are init vortices')
    parser.add_argument("--nu", type=float, default=0,
                        help='viscosity')
    parser.add_argument("--omega", type=float, default=0,
                        help='rotation speed of coordinate')
    parser.add_argument("--VP_num_x", type=float, default=1,
                        help='number of vortex particles in X direction')
    parser.add_argument("--VP_num_y", type=float, default=1,
                        help='number of vortex particles in Y direction')
    return parser
